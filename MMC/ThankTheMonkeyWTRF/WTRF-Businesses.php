 ...
 */
    ...
    class WTRF_BusinessesPage extends Page
    {
        protected function DoBeforeCreate()
        {
            $selectQuery = 'SELECT * from Business where mediaPartnerId = 1';
            $insertQuery = array('INSERT into Business
            (name,imageName,description,website,
            businessType,mediaPartnerId,videoId)
            VALUES
            (:name,:imageName,:description,:website,
            :businessType,1,:videoId)');
            $updateQuery = array('UPDATE business set
            name = :name,
            imageName = :imageName,
            ...
            $deleteQuery = array('DELETE from Business where primaryKey = :primaryKey');
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('WTRF-Businessesssearch', $this->dataset,
                array('primaryKey', 'name', 'imageName', 'description', 'website', 'businessType_name', 'videoId'),
                array($this->RenderText('PrimaryKey'), $this->RenderText('Name'), $this->RenderText('ImageName'), $this->RenderText('Description'), $this->RenderText('Website'), $this->RenderText('BusinessType'), $this->RenderText('VideoId')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('WTRF-Businessesasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
            ...
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('videoId', $this->RenderText('VideoId')));
        }
        ...
        protected function AddFieldColumns($grid)
        {
            ...
            //
            $editor = new TextEdit('videoid_edit');
        ...
        public function GetModalGridDeleteHandler() { return 'WTRF_Businesses_modal_delete'; }
        protected function GetEnableModalGridDelete() { return true; }
    
        protected function CreateGrid()
        {
            $result = new Grid($this, $this->dataset, 'WTRF_BusinessesGrid');
            if ($this->GetSecurityInfo()->HasDeleteGrant())
    ...
    try
    {
        $Page = new WTRF_BusinessesPage("WTRF-Businesses.php", "WTRF_Businesses", GetCurrentUserGrantForDataSource("WTRF_Businesses"), 'UTF-8');
        $Page->SetShortCaption('WTRF-Businesses');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('WTRF-Businesses');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("WTRF-Businesses"));

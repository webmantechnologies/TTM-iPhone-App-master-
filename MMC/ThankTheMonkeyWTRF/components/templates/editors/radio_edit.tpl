...
<label><input name="{$RadioEdit->GetName()}" value="{$Value}"{if $RadioEdit->GetSelectedValue() eq $Value} checked="checked"{/if} type="radio" {$Validators.InputAttributes}>{$Name}</label>

...
- (id) initWithFileName: (NSString *) fileName
{
    ...
    //NSString* path = [[NSBundle mainBundle] pathForResource:@"SteubenvilleOH" ofType:@"sqlite"];
	NSLog(@"initWithFileName: %@",[documentsDirectory stringByAppendingFormat:fileName]);
...
{
	int columnCount = sqlite3_column_count(statement);
	//NSLog(@"number of columns in statement: %d",columnCount);
...
- (int) typeForStatement: (sqlite3_stmt *) statement column: (int) column
{
	...
	//NSString *statement_text = [[NSString alloc] initWithCString:sqlite3_sql(statement) encoding:NSUTF8StringEncoding];
	if(columnType != NULL)
	{
        //NSLog(@"SQL Statement: %@",statement_text);
